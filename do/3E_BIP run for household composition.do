...
count if !missing(d_ghq_likert, bip, single_occ) & bip==1
local N_occ_b = r(N)

count if !missing(d_ghq_likert, bip, single_occ) & bip==0
local N_occ_nb = r(N)

* bip only
svy, subpop(if bip==1 & !missing(single_occ)): regress d_ghq_likert i.single_occ
margins single_occ, ///
    saving("${out}/Tables/hhcomp_tables/marg_single_occ_bip_cells.dta", replace)

* Non-bip only
svy, subpop(if bip==0 & !missing(single_occ)): regress d_ghq_likert i.single_occ
margins single_occ, ///
    saving("${out}/Tables/hhcomp_tables/marg_single_occ_nonbip_cells.dta", replace)

* pooled difference p-value
svy, subpop(if !missing(single_occ)): regress d_ghq_likert i.bip##i.single_occ
margins single_occ, dydx(bip) ///
...
count if !missing(d_ghq_likert, bip, kids_cat) & bip==1
local N_kids_b = r(N)

count if !missing(d_ghq_likert, bip, kids_cat) & bip==0
local N_kids_nb = r(N)

* bip only
svy, subpop(if bip==1 & !missing(kids_cat)): regress d_ghq_likert i.kids_cat
margins kids_cat, ///
    saving("${out}/Tables/hhcomp_tables/marg_kids_cat_bip_cells.dta", replace)

* Non-bip only
svy, subpop(if bip==0 & !missing(kids_cat)): regress d_ghq_likert i.kids_cat
margins kids_cat, ///
    saving("${out}/Tables/hhcomp_tables/marg_kids_cat_nonbip_cells.dta", replace)

* pooled difference p-value
svy, subpop(if !missing(kids_cat)): regress d_ghq_likert i.bip##i.kids_cat
margins kids_cat, dydx(bip) ///
...
**************************************************
* 4. Single occupancy: bip cells
**************************************************
use "${out}/Tables/hhcomp_tables/marg_single_occ_bip_cells.dta", clear
gen ord = _n

* 1 = Multi-occupancy, 2 = Single
...
**************************************************
* 5. Single occupancy: Non-bip cells
**************************************************
use "${out}/Tables/hhcomp_tables/marg_single_occ_nonbip_cells.dta", clear
gen ord = _n
...
**************************************************
* 7. Kids category: bip cells
**************************************************
use "${out}/Tables/hhcomp_tables/marg_kids_cat_bip_cells.dta", clear
gen ord = _n
...
**************************************************
* 8. Kids category: Non-bip cells
**************************************************
use "${out}/Tables/hhcomp_tables/marg_kids_cat_nonbip_cells.dta", clear
gen ord = _n
...
* 11. Excel
**************************************************
putexcel set "${out}/Tables/hhcomp_tables/Table_bip_hhcomp.xlsx", replace
