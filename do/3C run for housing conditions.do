...
count if !missing(d_ghq_likert, bame, bedratio_cat) & bame==1
local N_bed_b = r(N)

count if !missing(d_ghq_likert, bame, bedratio_cat) & bame==0
local N_bed_nb = r(N)

* BAME only
svy, subpop(if bame==1 & !missing(bedratio_cat)): regress d_ghq_likert i.bedratio_cat
margins bedratio_cat, ///
    saving("${out}/Tables/housing_tables/marg_bedratio_cat_bame_cells.dta", replace)

* Non-BAME only
svy, subpop(if bame==0 & !missing(bedratio_cat)): regress d_ghq_likert i.bedratio_cat
margins bedratio_cat, ///
    saving("${out}/Tables/housing_tables/marg_bedratio_cat_nonbame_cells.dta", replace)

* pooled difference p-value
svy, subpop(if !missing(bedratio_cat)): regress d_ghq_likert i.bame##i.bedratio_cat
margins bedratio_cat, dydx(bame) ///
...
count if !missing(d_ghq_likert, bame, hsrooms_cat) & bame==1
local N_room_b = r(N)

count if !missing(d_ghq_likert, bame, hsrooms_cat) & bame==0
local N_room_nb = r(N)

* BAME only
svy, subpop(if bame==1 & !missing(hsrooms_cat)): regress d_ghq_likert i.hsrooms_cat
margins hsrooms_cat, ///
    saving("${out}/Tables/housing_tables/marg_hsrooms_cat_bame_cells.dta", replace)

* Non-BAME only
svy, subpop(if bame==0 & !missing(hsrooms_cat)): regress d_ghq_likert i.hsrooms_cat
margins hsrooms_cat, ///
    saving("${out}/Tables/housing_tables/marg_hsrooms_cat_nonbame_cells.dta", replace)

* pooled difference p-value
svy, subpop(if !missing(hsrooms_cat)): regress d_ghq_likert i.bame##i.hsrooms_cat
margins hsrooms_cat, dydx(bame) ///
...
**************************************************
* 4. Bedroom ratio category: BAME cells
**************************************************
use "${out}/Tables/housing_tables/marg_bedratio_cat_bame_cells.dta", clear
gen ord = _n

* 假設排序：1 = <1 per person, 2 = 1-<2 per person, 3 = >=2 per person
forvalues i = 1/3 {
...
**************************************************
* 5. Bedroom ratio category: Non-BAME cells
**************************************************
use "${out}/Tables/housing_tables/marg_bedratio_cat_nonbame_cells.dta", clear
gen ord = _n
...
**************************************************
* 7. Other rooms category: BAME cells
**************************************************
use "${out}/Tables/housing_tables/marg_hsrooms_cat_bame_cells.dta", clear
gen ord = _n

* 假設排序：1 = 0-1 rooms, 2 = 2-3 rooms, 3 = 4+ rooms
forvalues i = 1/3 {
    quietly summarize _margin if ord==`i', meanonly
...
**************************************************
* 8. Other rooms category: Non-BAME cells
**************************************************
use "${out}/Tables/housing_tables/marg_hsrooms_cat_nonbame_cells.dta", clear
gen ord = _n
...
* 11. Excel
**************************************************
putexcel set "${out}/Tables/housing_tables/Table_BAME_housing.xlsx", replace
...
* notes
putexcel A25 = "Notes: Entries are margins from separate Bame-only and Non-Bame-only survey regressions. Standard errors are in parentheses. Difference reports p-values from pooled models testing Bame vs Non-Bame within each row."
