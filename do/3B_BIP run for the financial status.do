...
count if !missing(d_ghq_likert, bip, fin_diff_base) & bip==1
local N_finbase_b = r(N)

count if !missing(d_ghq_likert, bip, fin_diff_base) & bip==0
local N_finbase_nb = r(N)


* bip only
svy, subpop(if bip==1 & !missing(fin_diff_base)): regress d_ghq_likert i.fin_diff_base
margins fin_diff_base, ///
    saving("${out}/Tables/financial_tables/marg_fin_diff_base_bip_cells.dta", replace)

* Non-bip only
svy, subpop(if bip==0 & !missing(fin_diff_base)): regress d_ghq_likert i.fin_diff_base
margins fin_diff_base, ///
    saving("${out}/Tables/financial_tables/marg_fin_diff_base_nonbip_cells.dta", replace)

* pooled difference p-value
svy, subpop(if !missing(fin_diff_base)): regress d_ghq_likert i.bip##i.fin_diff_base
margins fin_diff_base, dydx(bip) ///
...
count if !missing(d_ghq_likert, bip, fin_change) & bip==1
local N_finchg_b = r(N)

count if !missing(d_ghq_likert, bip, fin_change) & bip==0
local N_finchg_nb = r(N)

* bip only
svy, subpop(if bip==1 & !missing(fin_change)): regress d_ghq_likert i.fin_change
margins fin_change, ///
    saving("${out}/Tables/financial_tables/marg_fin_change_bip_cells.dta", replace)

* Non-bip only
svy, subpop(if bip==0 & !missing(fin_change)): regress d_ghq_likert i.fin_change
margins fin_change, ///
    saving("${out}/Tables/financial_tables/marg_fin_change_nonbip_cells.dta", replace)

* pooled difference p-value
svy, subpop(if !missing(fin_change)): regress d_ghq_likert i.bip##i.fin_change
margins fin_change, dydx(bip) ///
...
**************************************************
* 4. Baseline financial difficulties: bip cells
**************************************************
use "${out}/Tables/financial_tables/marg_fin_diff_base_bip_cells.dta", clear
gen ord = _n
...
**************************************************
* 5. Baseline financial difficulties: Non-bip cells
**************************************************
use "${out}/Tables/financial_tables/marg_fin_diff_base_nonbip_cells.dta", clear
gen ord = _n
...
**************************************************
* 7. Financial change: bip cells
**************************************************
use "${out}/Tables/financial_tables/marg_fin_change_bip_cells.dta", clear
gen ord = _n
...
**************************************************
* 8. Financial change: Non-bip cells
**************************************************
use "${out}/Tables/financial_tables/marg_fin_change_nonbip_cells.dta", clear
gen ord = _n
...
* 11. 輸出到 Excel
**************************************************
putexcel set "${out}/Tables/financial_tables/Table_bip_financial.xlsx", replace
...
* notes
putexcel A23 = "Notes: Entries are margins from separate bip-only and Non-bip-only survey regressions. Standard errors are in parentheses. Difference reports p-values from pooled models testing bip vs Non-bip within each row."
