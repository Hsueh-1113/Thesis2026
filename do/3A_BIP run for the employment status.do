...
* 1. Employment change
**************************************************
count if !missing(d_ghq_likert, bip, emp_change) & bip==1
local N_emp_b = r(N)

count if !missing(d_ghq_likert, bip, emp_change) & bip==0
local N_emp_nb = r(N)

* bip only
svy, subpop(if bip==1 & !missing(emp_change)): regress d_ghq_likert i.emp_change
margins emp_change, saving("${out}/Tables/employment_tables/marg_emp_change_bip_cells.dta", replace)

* Non-bip only
svy, subpop(if bip==0 & !missing(emp_change)): regress d_ghq_likert i.emp_change
margins emp_change, saving("${out}/Tables/employment_tables/marg_emp_change_nonbip_cells.dta", replace)

* pooled difference p-value
svy, subpop(if !missing(emp_change)): regress d_ghq_likert i.bip##i.emp_change
margins emp_change, dydx(bip) ///
    saving("${out}/Tables/employment_tables/marg_emp_change_dydx.dta", replace)

**************************************************
* 2. Key worker
**************************************************
count if !missing(d_ghq_likert, bip, keyworker) & bip==1
local N_key_b = r(N)

count if !missing(d_ghq_likert, bip, keyworker) & bip==0
local N_key_nb = r(N)

* bip only
svy, subpop(if bip==1 & !missing(keyworker)): regress d_ghq_likert i.keyworker
margins keyworker, saving("${out}/Tables/employment_tables/marg_keyworker_bip_cells.dta", replace)

* Non-bip only
svy, subpop(if bip==0 & !missing(keyworker)): regress d_ghq_likert i.keyworker
margins keyworker, saving("${out}/Tables/employment_tables/marg_keyworker_nonbip_cells.dta", replace)

* pooled difference p-value
svy, subpop(if !missing(keyworker)): regress d_ghq_likert i.bip##i.keyworker
margins keyworker, dydx(bip) ///
    saving("${out}/Tables/employment_tables/marg_keyworker_dydx.dta", replace)

**************************************************
* 3. Self-employed
**************************************************
count if !missing(d_ghq_likert, bip, selfemp_only) & bip==1
local N_self_b = r(N)

count if !missing(d_ghq_likert, bip, selfemp_only) & bip==0
local N_self_nb = r(N)

* bip only
svy, subpop(if bip==1 & !missing(selfemp_only)): regress d_ghq_likert i.selfemp_only
margins selfemp_only, saving("${out}/Tables/employment_tables/marg_selfemp_bip_cells.dta", replace)

* Non-bip only
svy, subpop(if bip==0 & !missing(selfemp_only)): regress d_ghq_likert i.selfemp_only
margins selfemp_only, saving("${out}/Tables/employment_tables/marg_selfemp_nonbip_cells.dta", replace)

* pooled difference p-value
svy, subpop(if !missing(selfemp_only)): regress d_ghq_likert i.bip##i.selfemp_only
margins selfemp_only, dydx(bip) ///
...
**************************************************
* 5. employment status: bip cells
**************************************************
use "${out}/Tables/employment_tables/marg_emp_change_bip_cells.dta", clear
gen ord = _n
...
**************************************************
* 6. employment status: Non-bip cells
**************************************************
use "${out}/Tables/employment_tables/marg_emp_change_nonbip_cells.dta", clear
gen ord = _n
...
**************************************************
* 8. key worker: bip cells
**************************************************
use "${out}/Tables/employment_tables/marg_keyworker_bip_cells.dta", clear
gen ord = _n
...
**************************************************
* 9. key worker: Non-bip cells
**************************************************
use "${out}/Tables/employment_tables/marg_keyworker_nonbip_cells.dta", clear
gen ord = _n
...
**************************************************
* 11. self-employed: bip cells
**************************************************
use "${out}/Tables/employment_tables/marg_selfemp_bip_cells.dta", clear
gen ord = _n
...
**************************************************
* 12. self-employed: Non-bip cells
**************************************************
use "${out}/Tables/employment_tables/marg_selfemp_nonbip_cells.dta", clear
gen ord = _n
...
* 15. 輸出到 Excel
**************************************************
putexcel set "${out}/Tables/employment_tables/Table_bip_custom.xlsx", replace
...
* Notes
putexcel A31 = "Notes: Entries are margins from separate bip-only and Non-bip-only survey regressions. Standard errors are in parentheses. Difference reports p-values from pooled models testing bip vs Non-bip within each row."
