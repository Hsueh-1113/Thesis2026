...
count if !missing(d_ghq_likert, bame, sex_bin) & bame==1
local N_sex_b = r(N)

count if !missing(d_ghq_likert, bame, sex_bin) & bame==0
local N_sex_nb = r(N)

* BAME only
svy, subpop(if bame==1 & !missing(sex_bin)): regress d_ghq_likert i.sex_bin
margins sex_bin, ///
    saving("${out}/Tables/demo_tables/marg_sex_bin_bame_cells.dta", replace)

* Non-BAME only
svy, subpop(if bame==0 & !missing(sex_bin)): regress d_ghq_likert i.sex_bin
margins sex_bin, ///
    saving("${out}/Tables/demo_tables/marg_sex_bin_nonbame_cells.dta", replace)

* pooled difference p-value
svy, subpop(if !missing(sex_bin)): regress d_ghq_likert i.bame##i.sex_bin
margins sex_bin, dydx(bame) ///
...
count if !missing(d_ghq_likert, bame, agegrp) & bame==1
local N_age_b = r(N)

count if !missing(d_ghq_likert, bame, agegrp) & bame==0
local N_age_nb = r(N)

* BAME only
svy, subpop(if bame==1 & !missing(agegrp)): regress d_ghq_likert i.agegrp
margins agegrp, ///
    saving("${out}/Tables/demo_tables/marg_agegrp_bame_cells.dta", replace)

* Non-BAME only
svy, subpop(if bame==0 & !missing(agegrp)): regress d_ghq_likert i.agegrp
margins agegrp, ///
    saving("${out}/Tables/demo_tables/marg_agegrp_nonbame_cells.dta", replace)

* pooled difference p-value
svy, subpop(if !missing(agegrp)): regress d_ghq_likert i.bame##i.agegrp
margins agegrp, dydx(bame) ///
...
* 4. Sex: BAME cells
**************************************************
use "${out}/Tables/demo_tables/marg_sex_bin_bame_cells.dta", clear
gen ord = _n
...
**************************************************
* 5. Sex: Non-BAME cells
**************************************************
use "${out}/Tables/demo_tables/marg_sex_bin_nonbame_cells.dta", clear
gen ord = _n
...
**************************************************
* 7. Age group: BAME cells
**************************************************
use "${out}/Tables/demo_tables/marg_agegrp_bame_cells.dta", clear
gen ord = _n
...
**************************************************
* 8. Age group: Non-BAME cells
**************************************************
use "${out}/Tables/demo_tables/marg_agegrp_nonbame_cells.dta", clear
gen ord = _n
...
* 11. Excel
**************************************************
putexcel set "${out}/Tables/demo_tables/Table_BAME_demographics.xlsx", replace
