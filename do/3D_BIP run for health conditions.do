...
* 1. COVID symptoms
**************************************************
count if !missing(d_ghq_likert, bip, covid_symp) & bip==1
local N_covid_b = r(N)

count if !missing(d_ghq_likert, bip, covid_symp) & bip==0
local N_covid_nb = r(N)

svy, subpop(if bip==1 & !missing(covid_symp)): regress d_ghq_likert i.covid_symp
margins covid_symp, saving("${out}/Tables/health_tables/marg_covid_symp_bip_cells.dta", replace)

svy, subpop(if bip==0 & !missing(covid_symp)): regress d_ghq_likert i.covid_symp
margins covid_symp, saving("${out}/Tables/health_tables/marg_covid_symp_nonbip_cells.dta", replace)

svy, subpop(if !missing(covid_symp)): regress d_ghq_likert i.bip##i.covid_symp
margins covid_symp, dydx(bip) saving("${out}/Tables/health_tables/marg_covid_symp_dydx.dta", replace)

**************************************************
* 2. Respiratory chronic condition
**************************************************
count if !missing(d_ghq_likert, bip, cc_resp) & bip==1
local N_resp_b = r(N)

count if !missing(d_ghq_likert, bip, cc_resp) & bip==0
local N_resp_nb = r(N)

svy, subpop(if bip==1 & !missing(cc_resp)): regress d_ghq_likert i.cc_resp
margins cc_resp, saving("${out}/Tables/health_tables/marg_cc_resp_bip_cells.dta", replace)

svy, subpop(if bip==0 & !missing(cc_resp)): regress d_ghq_likert i.cc_resp
margins cc_resp, saving("${out}/Tables/health_tables/marg_cc_resp_nonbip_cells.dta", replace)

svy, subpop(if !missing(cc_resp)): regress d_ghq_likert i.bip##i.cc_resp
margins cc_resp, dydx(bip) saving("${out}/Tables/health_tables/marg_cc_resp_dydx.dta", replace)

**************************************************
* 3. Cardiovascular chronic condition
**************************************************
count if !missing(d_ghq_likert, bip, cc_cardio) & bip==1
local N_cardio_b = r(N)

count if !missing(d_ghq_likert, bip, cc_cardio) & bip==0
local N_cardio_nb = r(N)

svy, subpop(if bip==1 & !missing(cc_cardio)): regress d_ghq_likert i.cc_cardio
margins cc_cardio, saving("${out}/Tables/health_tables/marg_cc_cardio_bip_cells.dta", replace)

svy, subpop(if bip==0 & !missing(cc_cardio)): regress d_ghq_likert i.cc_cardio
margins cc_cardio, saving("${out}/Tables/health_tables/marg_cc_cardio_nonbip_cells.dta", replace)

svy, subpop(if !missing(cc_cardio)): regress d_ghq_likert i.bip##i.cc_cardio
margins cc_cardio, dydx(bip) saving("${out}/Tables/health_tables/marg_cc_cardio_dydx.dta", replace)

**************************************************
* 4. Endocrine chronic condition
**************************************************
count if !missing(d_ghq_likert, bip, cc_endo) & bip==1
local N_endo_b = r(N)

count if !missing(d_ghq_likert, bip, cc_endo) & bip==0
local N_endo_nb = r(N)

svy, subpop(if bip==1 & !missing(cc_endo)): regress d_ghq_likert i.cc_endo
margins cc_endo, saving("${out}/Tables/health_tables/marg_cc_endo_bip_cells.dta", replace)

svy, subpop(if bip==0 & !missing(cc_endo)): regress d_ghq_likert i.cc_endo
margins cc_endo, saving("${out}/Tables/health_tables/marg_cc_endo_nonbip_cells.dta", replace)

svy, subpop(if !missing(cc_endo)): regress d_ghq_likert i.bip##i.cc_endo
margins cc_endo, dydx(bip) saving("${out}/Tables/health_tables/marg_cc_endo_dydx.dta", replace)

**************************************************
* 5. Arthritis
**************************************************
count if !missing(d_ghq_likert, bip, cc_arth) & bip==1
local N_arth_b = r(N)

count if !missing(d_ghq_likert, bip, cc_arth) & bip==0
local N_arth_nb = r(N)

svy, subpop(if bip==1 & !missing(cc_arth)): regress d_ghq_likert i.cc_arth
margins cc_arth, saving("${out}/Tables/health_tables/marg_cc_arth_bip_cells.dta", replace)

svy, subpop(if bip==0 & !missing(cc_arth)): regress d_ghq_likert i.cc_arth
margins cc_arth, saving("${out}/Tables/health_tables/marg_cc_arth_nonbip_cells.dta", replace)

svy, subpop(if !missing(cc_arth)): regress d_ghq_likert i.bip##i.cc_arth
margins cc_arth, dydx(bip) saving("${out}/Tables/health_tables/marg_cc_arth_dydx.dta", replace)

**************************************************
* 6. Other chronic condition
**************************************************
count if !missing(d_ghq_likert, bip, cc_other) & bip==1
local N_other_b = r(N)

count if !missing(d_ghq_likert, bip, cc_other) & bip==0
local N_other_nb = r(N)

svy, subpop(if bip==1 & !missing(cc_other)): regress d_ghq_likert i.cc_other
margins cc_other, saving("${out}/Tables/health_tables/marg_cc_other_bip_cells.dta", replace)

svy, subpop(if bip==0 & !missing(cc_other)): regress d_ghq_likert i.cc_other
margins cc_other, saving("${out}/Tables/health_tables/marg_cc_other_nonv_cells.dta", replace)

svy, subpop(if !missing(cc_other)): regress d_ghq_likert i.bip##i.cc_other
margins cc_other, dydx(bip) saving("${out}/Tables/health_tables/marg_cc_other_dydx.dta", replace)
...
use "${out}/Tables/health_tables/marg_covid_symp_bip_cells.dta", clear
gen ord = _n
...
use "${out}/Tables/health_tables/marg_covid_symp_nonbip_cells.dta", clear
gen ord = _n
qui su _margin if ord==1, meanonly
...
* cc_resp: 0=No, 1=Yes
use "${out}/Tables/health_tables/marg_cc_resp_bip_cells.dta", clear
gen ord = _n
qui su _margin if ord==1, meanonly
...
use "${out}/Tables/health_tables/marg_cc_resp_nonbip_cells.dta", clear
gen ord = _n
qui su _margin if ord==1, meanonly
...
use "${out}/Tables/health_tables/marg_cc_cardio_bip_cells.dta", clear
gen ord = _n
qui su _margin if ord==1, meanonly
...
use "${out}/Tables/health_tables/marg_cc_cardio_nonbip_cells.dta", clear
gen ord = _n
qui su _margin if ord==1, meanonly
...
* 10. Excel：Table 4A
**************************************************
putexcel set "${out}/Tables/health_tables/Table_bip_health_A.xlsx", replace
...
putexcel A27 = "Notes: Entries are margins from separate bip-only and Non-bip-only survey regressions. Standard errors are in parentheses. Difference reports p-values from pooled models testing bip vs Non-bip within each row."
...
* cc_endo
use "${out}/Tables/health_tables/marg_cc_endo_bip_cells.dta", clear
gen ord = _n
qui su _margin if ord==1, meanonly
...
use "${out}/Tables/health_tables/marg_cc_endo_nonbip_cells.dta", clear
gen ord = _n
qui su _margin if ord==1, meanonly
...
* cc_arth
use "${out}/Tables/health_tables/marg_cc_arth_bip_cells.dta", clear
gen ord = _n
qui su _margin if ord==1, meanonly
...
use "${out}/Tables/health_tables/marg_cc_arth_nonbip_cells.dta", clear
gen ord = _n
qui su _margin if ord==1, meanonly
...
* cc_other
use "${out}/Tables/health_tables/marg_cc_other_bip_cells.dta", clear
gen ord = _n
qui su _margin if ord==1, meanonly
...
use "${out}/Tables/health_tables/marg_cc_other_nonbip_cells.dta", clear
gen ord = _n
qui su _margin if ord==1, meanonly
...
* 13. Excel：Table 4B
**************************************************
putexcel set "${out}/Tables/health_tables/Table_bip_health_B.xlsx", replace
